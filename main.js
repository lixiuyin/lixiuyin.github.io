...
(function () {
  ...
  // Follow the reading edge below the sticky bar, not the viewport midpoint:
  // short sections can share a screen. At the bottom, the last section cannot
  // always reach that edge, so select it explicitly. Coalesce layout reads.
  var navLinks = {};
  ...
  if (spyIds.length) {
    ...
    function updateScrollspy() {
      spyPending = false;
      var nextId = null;
      var edge = (topnav ? topnav.getBoundingClientRect().bottom : 0) + 8;
      spyIds.forEach(function (id) {
        if (document.getElementById(id).getBoundingClientRect().top <= edge) nextId = id;
      });
      if (window.scrollY > 0 && window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 2) {
        nextId = spyIds[spyIds.length - 1];
        // Several short sections can share the bottom viewport. Preserve an
        // explicit, still-visible section target; otherwise use the last one.
        var fragmentId = window.location.hash.slice(1);
        if (navLinks[fragmentId]) {
          var fragmentTop = document.getElementById(fragmentId).getBoundingClientRect().top;
          if (fragmentTop >= edge - 10 && fragmentTop < window.innerHeight) nextId = fragmentId;
        }
      }
      if (nextId === activeId) return;
      if (activeId) navLinks[activeId].removeAttribute("aria-current");
      activeId = nextId;
      if (activeId) navLinks[activeId].setAttribute("aria-current", "location");
    }
    ...
    // A different anchor at the same bottom scroll position emits no scroll.
    window.addEventListener("hashchange", scheduleScrollspy);
    // Expansion, details, fonts and image loading can move sections without
    // scrolling. Re-evaluate on content-size changes as well.
    if ("ResizeObserver" in window) {
      var contentResizeObserver = new ResizeObserver(scheduleScrollspy);
      contentResizeObserver.observe(document.querySelector("main"));
    }
    scheduleScrollspy();
  }
