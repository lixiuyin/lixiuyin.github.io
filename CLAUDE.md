...
Both the homepage and the CV exist to support **Research Assistant applications now** and a **future
PhD application** (reliable and self-improving LLM agents, feedback-based post-training and reward modeling,
synthetic data and environment synthesis, and causal or uncertainty-aware agent evaluation). The audience
is academic: faculty, PI labs, and admissions committees. This shapes content decisions:
...
GitHub Pages serves the **root of `main`** directly (`.nojekyll` disables Jekyll so files are served as-is).
Any push to `main` goes live in a minute or two. `make`/`make deploy` push to `main`, so a commit *is* a
deploy — there is no staging branch. Be deliberate about what you commit.

## Architecture & the one thing to get right

The site is three files plus assets:
- `index.html` — a sticky top nav, then all page content inline in semantic `<section>`s (bio, interests,
  publications, news, research, projects, education, awards, skills). **All styling is inlined in a `<style>`
...
- `assets/CV.tex` — the résumé, a **separate, parallel copy of the same career content** (based on the
  Aras Gungore LaTeX template). It is **local-only — deliberately NOT tracked or deployed** (so the LaTeX
  source can't be downloaded and reused); keep it on disk and back it up privately. `assets/CV.pdf` is the
  compiled output the page links to and **is committed** (GitHub Pages doesn't run LaTeX).
...
**`assets/CV.tex` is the source of truth.** The user edits the résumé; propagate career-content changes
*from* `CV.tex` *into* `index.html` (adapting phrasing — LaTeX bullets → HTML prose — while keeping the
facts identical). Then run `make cv` so `CV.pdf` reflects the `.tex` changes before deploying.

**Sync is strictly one-way.** Never back-propagate into `CV.tex` — not page/CSS changes, and **not even a
*shared fact* edited directly in `index.html`** (a metric, a wording fix, contact info). If a shared fact
was changed on the site, don't silently mirror it back; flag it and ask the user to make the edit in
`CV.tex`, which stays the sole source.
...
**Selected Projects layout.** The `#projects` section opens with a one-line ownership sentence
(`<p class="muted">`, role framing), then the strongest projects as full `.entry` blocks — each carrying an
`.entry-meta` role/date label (`Project Lead`, `Personal`, …). Lower-signal or older work is
folded into a single trailing `.entry` titled *Additional projects*: a compact, role-neutral bulleted list
(`ul.more-projects`), one project per line with one concrete technical highlight. That list is the usual home
...
- **Site-only — the CV has no equivalent:** the bio paragraph, the News section, the Publications
  placeholder, and the *Personal* (hobbies) line. These live only in `index.html`.
...
`.gitignore` ignores everything under `assets/` **except** `*.pdf`, `*.woff2`, and common image
formats. LaTeX build artifacts are never committed by accident, and `CV.tex` is **intentionally excluded**
(local-only source — only the compiled `CV.pdf` is public). If you add a new asset type that *should* ship
(e.g. a `.bib`), add an allowlist exception or it won't be tracked or deployed.

- **Two portraits, both needed.** The page loads `photo.jpg` (~44 KB, web). `CV.tex` includes the photo
  *extension-less* (`\includegraphics{photo}`), which resolves to the high-res `photo.png` (~1.7 MB) for
  print — so keep both; don't delete `photo.png` as "redundant."
